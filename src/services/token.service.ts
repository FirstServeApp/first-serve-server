...
export const generateTokens = async (user: IUser | ISocialUser, session?: any): Promise<TokensPair> => {
  try {
    ...
    const accessToken = await jwt.sign(payload, accessSecret, { expiresIn: getEnv('JWT_ACCESS_LIFETIME') || '30s' })
    const refreshToken = await jwt.sign(payload, refreshSecret, { expiresIn: getEnv('JWT_REFRESH_LIFETIME') || '30d' })
